...
// newRulesCmd creates a new `fic firewalls rules` command
func newRulesCmd(clientFn func() (*fic.ServiceClient, error), out io.Writer) *cobra.Command {
	var routerID string

	cmd := &cobra.Command{
		Use:     "rules <id>",
		Short:   "Show Rules of Firewall",
		Example: "fic firewalls rules F040123456789 --router F022000000335",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, v := range fw.Rules {
				t.AddLine(v.From, v.To, v.Entries)
			}
			t.Print()
			return nil
		},
