...
func TestDeactivateCmd(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			testdata.DeactivateRequest,
			http.StatusAccepted,
			testdata.DeactivateResponse,
			testdata.DeactivateOutput,
			"",
		},
		{
			"client auth error",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.statusCode != 0 {
				...
				th.Mux.HandleFunc(
					fmt.Sprintf("/routers/%s/nats/%s/deactivate", routerID, natID),
