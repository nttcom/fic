...
func TestDestinationCmd(t *testing.T) {
	const connectionID = "F032000001482"
	...
	testCases := []struct {
	...
	}{
		{
			...
			nil,
			http.StatusOK,
			testdata.DestinationResponse,
			testdata.DestinationOutput,
			"",
		},
		{
			...
			nil,
			http.StatusOK,
			testdata.DestinationPairedResponse,
			testdata.DestinationPairedOutput,
			"",
		},
		{
			"client auth error",
		...
		},
		{
			...
			"",
			"",
			"calling Show router to port connection API:",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd.SetArgs([]string{connectionID})
			err := cmd.Execute()
