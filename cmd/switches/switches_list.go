	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				...
				if errors.As(err, &e) {
					return fmt.Errorf("extracting switches from API response: %w", err)
				}
			...
			t.AddHeader("id", "switchName", "area", "location", "portTypes", "numberOfAvailableVLANs")
			for _, sw := range sws {
				t.AddLine(sw.ID, sw.SwitchName, sw.Area, sw.Location, sw.PortTypes, sw.NumberOfAvailableVLANs)
			}
			t.Print()
